...
const GameState = {
    ...
    increaseScore() {
        // 🔧 使用统一配置中的得分值，而不是硬编码
        const scorePerCorrect = (typeof getConfig !== 'undefined') ? getConfig('game.scorePerCorrect') : 1;
        this.score += scorePerCorrect;
        return this.score;
    },
    ...
    decreaseScore() {
        // 🔧 扣分也使用配置中的值
        const scorePerCorrect = (typeof getConfig !== 'undefined') ? getConfig('game.scorePerCorrect') : 1;
        this.score = Math.max(0, this.score - scorePerCorrect);
        return this.score;
    },
...
const GameLogic = {
    ...
    showLevelCompleteModal() {
        ...
        UI.modalMessageEl.innerHTML = `
            <h3>当前总分: ${GameState.score}</h3>
            <p>已获得: ${itemsSummary}</p>
        `;
